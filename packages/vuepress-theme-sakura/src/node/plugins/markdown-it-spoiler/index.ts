...
export function markdownItSpoiler(
  md: Markdown,
  options: MarkdownItSpoilerOptions = {}
) {
  const config = {
...
const tokenize = (config: MarkdownItSpoilerOptions) => (state, silent) => {
...
const postProcess = (state, delimiters, config: MarkdownItSpoilerOptions) => {
  ...
  while (loneMarkers.length) {
    const i = <number>loneMarkers.pop()
    let j = i + 1
    while (
