...
module.exports = ctx => {
  ...
  Post.method('setCategories', function (this: typeof Post, cats) {
    ...
    const id = this._id;
    const allIds: string[] = [];
    const existed = PostCategory.find({post_id: id}, {lean: true}).map(data => data._id);
    ...
    return (hasHierarchy ? Promise.each(cats, addHierarchy) : Promise.resolve(addHierarchy(cats))
      ...
      existed.filter(item => !postCats.map(data => data._id).includes(item))).map(cat => PostCategory.removeById(cat));
  });
  ...
  Post.method('setTags', function (this: typeof Post, tags) {
    ...
    const Tag = ctx.database.model('Tag');
    const id = this._id;
    const existed = PostTag.find({post_id: id}, {lean: true}).map(data => data._id);
    ...
    return Promise.map(tags, tag => {
      ...
      // Remove old tags
      return existed.filter(item => !tags.map(data => data._id).includes(item));
